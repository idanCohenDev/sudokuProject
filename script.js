...
document.querySelector(".best-score-name").textContent =
  localStorage.getItem("name");
if (localStorage.getItem("time") === null) {
  // Preventing "null seconds" to be shown
...
function makeGrid() {
  ...
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
      ...
      if (i === 2 || i === 5 || i === 8) {
        // Checking if the borders should be bold
...
function makeMobileButtons() {
  // Making the buttons for mobile view
  const mobileNumbersContainer = document.querySelector(".mobile-numbers-container");
  for (let i = 0; i < 10; i++) {
    // Creating 9 digits containers each with his own number
...
function checkInput() {
  // Checking if the inputs evaluate to the correct username and password
  if (
    userNameInput.value.toLowerCase() === "abcd" &&
    passwordInput.value === "1234"
  ) {
    loginContainer.style.display = "none";
    difficultyContainer.style.display = "flex";
  } else {
    // Show the user an error and turn the borders of the inputs to red
...
mainContainer.addEventListener("keydown", ({ key }) => {
  // Adding an event listener to the "Enter" key as well as the click for better experience
  if (key === "Enter") {
    checkInput();
  }
});
...
function generateNumbers(difficultyValue) {
  for (let i = 0; i < Math.floor(81 * difficultyValue); i++) {
    // Generating numbers to be shown on the grid depending on the difficulty chosen
    ...
    if (!lockedCube.textContent) {
      ...
      // If the random cube already has text go back one iteration and skip it
      i--;
      continue;
    }
  }
...
async function startGame() {
  // Making the function asynchronous so it will wait for the game board
  difficultyContainer.style.display = "none";
  document.querySelector(".loading").style.display = "block";
  try {
    // Trying to get a random board from the sudoku api from rapid
    const config = {
      headers: {
  ...
  switch (
    this.value // depending of the value of the difficulty button clicked we generate numbers. e.g: if the value is easy we'll generate 81 * 0.75 numbers.
  ) {
...
function chooseNumber({ key }) {
  // Choosing numbers for computer by clicking on the keyboard
  ...
  if ((key > 0 && key < 10) || key === "Backspace") {
    // Checking if the user input is equivalent to a reasonable value
    if (key === "Backspace") {
      // Giving the user to delete his input
...
function addNumber() {
  //Choosing numbers for mobile / tablet by clicking in the numbers container
...
function focus({ type }) {
  removeStyles();
  highlightNumbers(this.textContent);
  if (!this.classList.contains("locked-cube")) {
    //Checking if the chosen cube is not a "locked" cube
    this.classList.add("focused-cube");
    if (nightModeFlag) {
      this.classList.add("night-mode-focused-cube");
    }
    for (let i = 0; i < 81; i++) {
      // Focusing on the rows and the columns by giving them a class that changes the background
      if (gameCubes[i].id === this.id) {
        // A statement which lets me differentiate between focused cube to it's row / column.
        continue;
      } else if (
        gameCubes[i].id.charAt(0) === this.id.charAt(0) ||
        gameCubes[i].id.charAt(2) === this.id.charAt(2)
      ) {
        // Checking for the cubes in the same row / column as my chosen cube
        gameCubes[i].classList.add("focused-row-column");
        if (nightModeFlag) {
          gameCubes[i].classList.add("night-mode-focused-row-column");
        }
        for (let j = 0; j < 9; j++) {
          // Checking what "box" the current focused cube is in
          if (this.classList.contains(`${j}`)) {
            ...
            for (let k = 0; k < 9; k++) {
              // Giving each cube in the box a focused class
              if (bigCubeCubes[k].classList.contains("focused-cube")) {
                // Skipping the focused cube
                continue;
              }
              if (nightModeFlag) {
...
function restartChoice() {
  ...
  if (this.value === "yes") {
    ...
    for (let i = 0; i < 81; i++) {
      if (gameCubes[i].classList.contains("locked-cube")) {
        // If the is "locked" then skip it
        continue;
      } else {
        // If the cube isn't locked then delete the number inside
...
function removeStyles() {
  // Removing all the focused styling to reset them
  for (let i = 0; i < 81; i++) {
...
function finishGame() {
  ...
  if (counter === 81) {
    // If the counter evaluate to 81 so the players win.
    chooseBest(); // Triggering the current best score chooser in case of finish
    for (let i = 0; i < 81; i++) {
      // Looping through the correct cubes array to make a green winning effect
      gameCubes[i].classList.add("correct-cube");
    }
    finalTimeDisplay.textContent = time;
    setTimeout(function () {
      // After the animation finished (2 seconds) display the finished game pop up
      popUp(finishMessagePopUp);
    }, 1000);
  } else {
    // If there are less then 81 correct instances than show a message with the number of mistakes
    mistakesMessage.style.display = "block";
    mistakesNumber.textContent = 81 - counter;
    setTimeout(function () {
      // Clear the mistakes message after 2 seconds
...
function resetGame() {
  // Resetting the game if the user clicks the return arrow or finishes
  removeStyles();
  for (let i = 0; i < 81; i++) {
    gameCubes[i].textContent = "";
...
function tryAgain() {
  // if the user finishes and decides to try again reset the game and return the style to normal and take him back to the difficulty page
  gameContainer.style.filter = "blur(0px)";
  settingsContainer.style.filter = "blur(0px)";
...
function chooseBest() {
  if (Number(localStorage.getItem("time")) > time || localStorage.length === 0) {
    // Checking if the time which is currently saved in the local storage is bigger than the current time.
